...
class ClashMeta extends AbstractProtocol
{
    ...
    public function handle()
    {
        ...
        foreach ($servers as $item) {
            ...
            if ($item['type'] === Server::TYPE_GHOSTPASS) {
                array_push($proxy, self::buildGhostPass($item['password'], $item));
                array_push($proxies, $item['name']);
            }
    ...
    public static function buildGhostPass($password, $server)
    {
        $protocol_settings = data_get($server, 'protocol_settings', []);
        $array = [
            'name' => $server['name'],
        ...
        // GhostPass specific options
        $array['shaper-enabled'] = true;

        // Traffic profile (overrides individual shaper settings)
        if ($trafficProfile = data_get($protocol_settings, 'traffic_profile')) {
            $array['traffic-profile'] = $trafficProfile;
        }

        if ($paddingRate = data_get($protocol_settings, 'padding_rate')) {
            $array['padding-rate'] = (int) $paddingRate;
        }
        if ($minPaddingSize = data_get($protocol_settings, 'min_padding_size')) {
            $array['min-padding-size'] = (int) $minPaddingSize;
        }
        if ($maxPaddingSize = data_get($protocol_settings, 'max_padding_size')) {
            $array['max-padding-size'] = (int) $maxPaddingSize;
        }
        if ($maxWriteSize = data_get($protocol_settings, 'max_write_size')) {
            $array['max-write-size'] = (int) $maxWriteSize;
        }

        // Advanced timing jitter (anti-ML fingerprinting)
        if ($timingJitter = data_get($protocol_settings, 'timing_jitter')) {
            $array['timing-jitter'] = (bool) $timingJitter;
        }
        if ($minJitterMs = data_get($protocol_settings, 'min_jitter_ms')) {
            $array['min-jitter-ms'] = (int) $minJitterMs;
        }
        if ($maxJitterMs = data_get($protocol_settings, 'max_jitter_ms')) {
            $array['max-jitter-ms'] = (int) $maxJitterMs;
        }
        if ($jitterRate = data_get($protocol_settings, 'jitter_rate')) {
            $array['jitter-rate'] = (int) $jitterRate;
        }
        if ($burstSize = data_get($protocol_settings, 'burst_size')) {
            $array['burst-size'] = (int) $burstSize;
        }
        if ($burstPauseMs = data_get($protocol_settings, 'burst_pause_ms')) {
            $array['burst-pause-ms'] = (int) $burstPauseMs;
        }
